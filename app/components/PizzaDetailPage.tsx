  ...
  return (
    <div className="k-shell">
      ...
      <div className="hidden md:flex px-5 py-2.5 border-b border-white/50 items-center gap-3 topbar shrink-0">
        ...
        <span className="font-display font-bold text-[15px] text-slate-900">Pizza {formatDate(order.date)}</span>
        <span
        ...
        {totalCount > 0 && (
          <span className="ml-auto font-display font-bold text-[16px] text-slate-900">{totals.finalTotal} Kč</span>
        )}
      </div>
      ...
      <div className="md:hidden border-b border-white/50 topbar shrink-0">
        <div className="flex items-center gap-3 px-4 py-2.5">
          ...
          <span className="font-display font-bold text-[14px] text-slate-900 flex-1">Pizza {formatDate(order.date)}</span>
          {totalCount > 0 && (
            <span className="font-display font-bold text-[14px] text-slate-900">{totals.finalTotal} Kč</span>
          )}
        </div>
                ...
                return (
                  <div key={row.id} className="border-b border-white/30 last:border-0">
                    ...
                    <div className="md:hidden flex items-center gap-3 px-4 py-3">
                      <span className="font-mono text-[11px] text-slate-400 w-5 shrink-0">{idx + 1}</span>
                      <div className="flex-1 min-w-0">
                        <p className="text-[13px] font-semibold text-slate-800">{row.personName || "–"}</p>
                        <p className="text-[12px] text-slate-500 truncate">
                          {row.pizzaItem ? `${row.pizzaItem.code}. ${row.pizzaItem.name}` : "–"}
                        </p>
                      </div>
                      <div className="text-right shrink-0">
                        <p className="text-[12px] text-slate-400">{row.count} ks</p>
                        <p className="text-[12px] font-semibold text-slate-700">{adjustedPrice > 0 ? `${adjustedPrice} Kč` : row.rowPrice > 0 ? `${row.rowPrice} Kč` : "–"}</p>
                      </div>
                    </div>
                    {/* Desktop */}
                    <div className="hidden md:grid items-center gap-3 px-4 py-2.5 text-[12.5px]" style={{ gridTemplateColumns: "28px 1fr 2fr 60px 80px 80px" }}>
                      ...
                      <span className="text-slate-600">{row.pizzaItem ? `${row.pizzaItem.code}. ${row.pizzaItem.name}` : "–"}</span>
                      <span className="text-center text-slate-600">{row.count}</span>
                      <span className="text-right text-slate-500">{row.rowPrice > 0 ? `${row.rowPrice} Kč` : "–"}</span>
                      <span className="text-right font-semibold text-slate-800">{adjustedPrice > 0 ? `${adjustedPrice} Kč` : "–"}</span>
                    </div>
                  </div>
                );
              })}
